...
Kij=.2*ones(N); % Matrix interaction Kij

Ki=.5*ones(N,1); % death rate

t0=0; T=1000; % t0 is start time, and T is the end time

s = RandStream('twister','Seed',123); % seed for generating random numbers
...
xx=x./(ones(N,1)*sum(x)); % relative abundances

figure

pb=plot(t(1:20:end),xx(Blue,1:20:end),'b'); % plot Blue species
set(pb,'LineWidth',2)
hold on

pr=plot(t(1:20:end),xx(Red,1:20:end)); % plot Red species
set(pr,'LineWidth',2,'color',PcR)

pg=plot(t(1:20:end),xx(Green,1:20:end)); % plot Green species
